...
class _PSPModule(nn.Module):
    ...
    def forward(self, features):
        ...
        for stage in self.stages:
            score.append(stage(features))
            pyramids.extend([F.interpolate(stage(features), size=(h, w), mode='bilinear', align_corners=True)])
       # newfeat=F.interpolate(score[0], size=(2, 2), mode='bilinear', align_corners=True)+score[1]
       # newfeat=F.interpolate(newfeat, size=(3, 3), mode='bilinear', align_corners=True)+score[2]
       # newfeat=F.interpolate(newfeat, size=(6, 6), mode='bilinear', align_corners=True)+score[3]
       # pyramids.extend([F.interpolate(newfeat, size=(h, w), mode='bilinear', align_corners=True)])
...
class PSPNet(nn.Module):
    def __init__(self, num_classes, downsample_factor, backbone="resnet50", pretrained=True, aux_branch=True):
        ...
        self.master_branch_2 = nn.Sequential(
            _PSPModule(out_channel_2, pool_sizes=[1, 2, 3, 6], norm_layer=norm_layer),
            nn.Conv2d(out_channel_2 // 4, num_classes, kernel_size=1)
        )
        self.vit = vit()
        ...
        self.initialize_weights(self.master_branch_2)

    def forward(self, x):
        input_size = (x.size()[2], x.size()[3])
        vx = F.interpolate(x, size=(480, 480), mode='bilinear', align_corners=True)
        vit_x = self.vit(vx)
        x_aux, x, x_2 = self.backbone(x)
        x=vit_x+x
        #x=self.vit(x)
        output = self.master_branch(x)
        output_2=self.master_branch_2(x_aux)
        output=F.interpolate(output,size=output_2.size()[2:],mode='bilinear',align_corners=True)
