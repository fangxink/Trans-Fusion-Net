...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):  
    ...
    for ind in range(len(gt_imgs)): 
        ...
        jaccard+=jaccard_score(label.flatten(),pred.flatten(),average='micro')
...
if __name__ == "__main__":
    gt_dir = "./dataset/SegmentationClass"
    pred_dir = "./miou_pr_dir"
    png_name_list = open("./dataset/ImageSets/Segmentation/test.txt",'r').read().splitlines()
    #------------------------------#
    #   分类个数+1
    #   2+1
    #------------------------------#
    ...
    #--------------------------------------------#
    name_classes = ["background","pterygium"]
